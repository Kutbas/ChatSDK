...
// 处理流式响应（支持多会话后台生成）
async function processStreamResponse(response, messageId, sessionId) { // 【新增 sessionId 参数】
        ...
        while (true) {
            ...
            while ((lineEnd = buffer.indexOf('\n')) !== -1) {
                ...
                if (line.startsWith('data: ')) {
                    ...
                    if (data === '[DONE]') {
                        if (unparsedBuffer) {
                            streamState.fullContent += unparsedBuffer; 
                            unparsedBuffer = '';
                        }
                    ...
                    try {
                        ...
                        if (processedData.startsWith('{') && processedData.endsWith('}')) {
                            ...
                            const extracted = jsonData.content || jsonData.text || jsonData.message || '';
                        ...
                        if (isAlreadyDecoded) {
                            streamState.fullContent += processedData;
                        } else {
                            if (processedData.startsWith('"') && processedData.endsWith('"')) {
                                processedData = processedData.slice(1, -1);
                            }
                            unparsedBuffer += processedData;
                            
                            unparsedBuffer = unparsedBuffer
                                .replace(/\\\\/g, '\\')
                                .replace(/\\"/g, '"')
                                .replace(/\\n/g, '\n')
                                .replace(/\\t/g, '\t')
                                .replace(/\\r/g, '\r')
                                .replace(/\\u([\dA-Fa-f]{4})/gi, (match, p1) => {
                                    try { return String.fromCharCode(parseInt(p1, 16)); } 
                                    catch (e) { return match; }
                                });

                            const trailingMatch = unparsedBuffer.match(/\\(?:u[\dA-Fa-f]{0,3})?$/i);
                            if (trailingMatch) {
                                const splitIndex = unparsedBuffer.length - trailingMatch[0].length;
                                streamState.fullContent += unparsedBuffer.substring(0, splitIndex);
                                unparsedBuffer = unparsedBuffer.substring(splitIndex);
                            } else {
                                streamState.fullContent += unparsedBuffer;
                                unparsedBuffer = '';
                            }
